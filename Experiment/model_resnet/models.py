...
class ResNet(nn.Module):
    ...
    Args:
        ...
        scalar_input_num: how many scalar input features (sex, race) are there? default to 1.
        MLP_hidden_layer_sizes: number of neurons in the hidden layer, default to None
    ...
    def __init__(
        ...
        scalar_input_num: int = 1,  # how many scalar input features are there?
        MLP_hidden_layer_sizes: list[int] | None = None,  # number of neurons in the hidden layer
        ...
        # Chenyu: feedforward network at the end
        self.scalar_input_num = scalar_input_num
        MLP_input_size = block_inplanes[3] * block.expansion + scalar_input_num
        if MLP_hidden_layer_sizes == None:
            self.mlp = nn.Linear(MLP_input_size, num_classes)
        else:
            MLP_layers = []
    ...
    def forward(self, x: torch.Tensor, x_scalar: torch.Tensor | None = None) -> torch.Tensor:
        ...
        # Chenyu: feedforward network
        x = x.view(x.size(0), -1)
        if (x_scalar == None) and (self.scalar_input_num == 0):
            x = self.mlp(x)
        else:
            x_scalar = x_scalar.view(x_scalar.size(0), -1)
            if x_scalar.size(1) != self.scalar_input_num:
                raise ValueError("Chenyu: x_scalar dimension mismatch with scalar_input_num")
            else:
                x = torch.concat((x, x_scalar), dim=1)
                x = self.mlp(x)
